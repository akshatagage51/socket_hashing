...
int main()
{
	...
	ptr=(struct iptuppel *)malloc(sizeof(struct iptuppel));
	/*creating TCP socket*/
	...
	if (bind(u_serverfd, (struct sockaddr* ) &my_addr, sizeof(my_addr))==-1)
	      printf("bind error\n");
	else
	...
	while(1)
	{
		...
		if(FD_ISSET(server_sockfd,&readfds))
		{
			...
			strcpy(ptr->proto,"TCP");
			/*getsockname is used to know who is there at the other end of connected stream*/
			getsockname(server_sockfd,(struct sockaddr *)&server_address,&server_len);

			/*getpeername is used to get ip address of local  machine*/
			getpeername(client_sockfd,(struct sockaddr *)&client_address,&client_len);

			strcpy(ptr->d_ip,inet_ntoa(server_address.sin_addr));
			strcpy(ptr->s_ip,inet_ntoa(client_address.sin_addr));
			...
			if(strcmp(ch,"lookup")==0)
				lookup_packet((void *)opnadd,ptr);
			if(strcmp(ch,"delete")==0)
			{
                                ret=delete_packet((void *)opnadd,ptr);
				if(ret==0)
					printf("packet deleted sucessfully\n");
				else
					printf("packet not deleted\n");
			}
	}
		...
		if(FD_ISSET(u_serverfd,&readfds))
		{
			...
			strcpy(ptr->d_ip,inet_ntoa(my_addr.sin_addr));

			getpeername(u_serverfd,(struct sockaddr *)&cli_addr,&len);
			strcpy(ptr->s_ip,inet_ntoa(cli_addr.sin_addr));
